...
sudo bash -c "touch ~/gnb_logs_$1"  # expect an experiment ID passed as an argument to the sh script.
xterm -e bash -c "cd /opt/openairinterface5g/cmake_targets; sudo ./ran_build/build/nr-softmodem --sa -O /local/repository/etc/gnb.conf -d  > ~/gnb_logs_$1 2>&1 &" &