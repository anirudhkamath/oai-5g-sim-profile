...
install_grafana () {
    ...
    sudo apt-get update
    sudo apt-get -y install --no-install-recommends grafana sqlite crudini
...
install_gnb_service () {
    echo "installing oai-gnb service"
    sudo cp $SERVICESDIR/oai-gnb.service /etc/systemd/system/
    sudo systemctl daemon-reload
    ## We don't necessarily want to start the service right away
    # sudo systemctl enable oai-gnb
    # sudo systemctl restart oai-gnb
}
